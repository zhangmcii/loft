<template>
  <van-cell :border="false" :to="`/user/${follows.username}`">
    ...
    <template #right-icon v-if="showFollowButton">
      <van-icon class="icon" name="add" color="blue" :size="25" v-if="!isFollowed" @click.stop="followUser" />
      <van-icon class="icon" name="checked" color="gray" :size="25" v-if="isFollowed && !isMutualFollow"
        @click.stop="unFollowUser" />
      <van-icon class="icon" name="sort" color="gray" :size="25" v-if="isMutualFollow" @click.stop="unFollowUser" />
    </template>
  </van-cell>
</template>
