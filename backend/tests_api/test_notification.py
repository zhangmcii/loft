...
class TestNotificationCase:
    ...
    def test_comment_notification(self, client, auth):
        ...
        # 发布文章
        r = client.post(self.pre_fix + '/posts', headers=author_headers,
        ...
        # 第二个用户评论第一个用户的文章
        r = client.post(self.pre_fix + f'/posts/{post_id}/comments', headers=auth.get_headers(),
    ...
    def test_reply_notification(self, client, auth):
        ...
        # 发布文章
        r = client.post(self.pre_fix + '/posts', headers=commenter1_headers,
                        json={'body': '测试文章内容', 'bodyHtml': '测试文章内容', 'images': []})
        assert r.json.get('code') == 200
        post_id = r.json.get('data')[0].get('id')
    ...
    def test_mark_notification_as_read(self, client, auth):
        ...
        # 标记通知为已读
        r = client.patch(self.pre_fix + '/notifications', headers=author_headers, json={'ids': [notification_id]})
        assert r.status_code == 200
        assert r.json.get('code') == 200
