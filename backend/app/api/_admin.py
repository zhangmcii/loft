...
def post_init_summary():
    ...
    如果文章的summary为空，则使用truncate_markdown方法生成summary
    """
    try:
        # 查询所有summary为空的文章
        ...
        for post in posts_without_summary:
            try:
                ...
                if content:
                    summary = Post.truncate_markdown(content)
                    # 额外安全检查：确保不超过数据库字段长度(500)
                    # if len(summary) > 499:  # 留1个字符的余地
                    #     summary = summary[:498].rstrip() + "…"
                    logging.info(f"为文章长度： {len(content)} ")
