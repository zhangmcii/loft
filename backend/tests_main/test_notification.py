...
class TestNotificationCase:
    ...
    def test_comment_notification(self, client, auth):
        ...
        # 发布文章
        r = client.post('/', headers=author_headers, json={'body': '测试文章内容', 'bodyHtml': '测试文章内容', 'images': []})
        ...
        # 第二个用户评论第一个用户的文章
        r = client.post(f'/post/{post_id}', headers=auth.get_headers(), json={'body': '这是一条测试评论', 'directParentId': None, 'at': [] })
    ...
    def test_like_notification(self, client, auth):
        ...
        # 第二个用户点赞第一个用户的文章
        r = client.post(f'/praise/{post_id}', headers=liker_headers)
        assert r.status_code == 200
        assert r.json.get('code') == 200
    ...
    def test_reply_notification(self, client, auth):
        ...
        # 发布文章
        r = client.post('/', headers=commenter1_headers, json={'body': '测试文章内容', 'bodyHtml': '测试文章内容', 'images': []})
        assert r.json.get('code') == 200
        post_id = r.json.get('data')[0].get('id')

        # 发表评论
        r = client.post(f'/post/{post_id}', headers=auth.get_headers(), json={'body': '这是一条评论', 'directParentId': None, 'at': [] })
        assert r.status_code == 200
        ...
        # 第二个用户回复第一个用户的评论
        r = client.post(f'/post/{post_id}', headers=auth.get_headers(), json={'body': '这是对评论的回复', 'directParentId': comment_id, 'at': [] })
        assert r.status_code == 200
        assert r.json.get('code') == 200

        # 第一个用户查看通知
        r = client.get('/notifications', headers=commenter1_headers)
        assert r.status_code == 200
        assert r.json.get('code') == 200
    ...
    def test_mark_notification_as_read(self, client, auth):
        ...
        # 标记通知为已读
        r = client.post('/notification/read', headers=author_headers, json={'ids': [notification_id]})
        assert r.status_code == 200
        assert r.json.get('code') == 200
        ...
        r = client.get('/notifications', headers=author_headers)
        assert r.status_code == 200
        assert r.json.get('data')[-1].get('isRead') == True