...
class TestPraiseCommentCase:
    ...
    def test_post_comment_and_praise(self, client, auth):
        ...
        # 发布一篇文章
        r = client.post('/', headers=auth.get_headers(), json={'body': '测试文章内容', 'bodyHtml': '测试文章内容', 'images': []})
    ...
    def test_comment_reply_and_praise(self, client, auth):
        ...
        # 发表根评论
        r = client.post(f'/post/{post_id}', headers=auth.get_headers(), json={'body': '这是一条根评论', 'directParentId': None, 'at': [] })
        assert r.status_code == 200
        comment_id = r.json.get('data').get('id')

        # 回复评论
        r = client.post(f'/post/{post_id}', headers=auth.get_headers(), json={'body': '这是对根评论的回复', 'directParentId': comment_id, 'at': [] })
        assert r.status_code == 200
        assert r.json.get('code') == 200

        # 获取评论的回复
        r = client.get(f'/api/v1/reply_comments/?rootCommentId={comment_id}', headers=auth.get_headers())
        assert r.status_code == 200
        assert r.json.get('code') == 200
        assert len(r.json.get('data')) > 0
